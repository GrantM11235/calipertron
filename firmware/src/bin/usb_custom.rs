...
async fn main(_spawner: Spawner) {
    ...
    // If I just do this, then writing USB commands to change the timer frequency results in the system hanging (ADC streaming stops, no defmt messages)
    // join(fut_commands, join(fut_usb, fut_stream_adc)).await;

    // But if I also add this ping task, everything works fine.
    let fut_ping = async {
        loop {
            info!("ping");
...
static SIGNAL: [u32; 1] = [0b00000000000000000000000000000000];
